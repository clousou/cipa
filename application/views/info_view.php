			...
			if($mdata->num_rows()>0){
				...
				foreach($mdata->result_array() as $rows){
					...
					if($rows['status']=='DRAFT'){
						$button='<a class="btn btn-sm btn-danger" href="'.site_url('info/publish/'.trim(base64_encode($rows['id']),'=').'').'">
									<i class="fa fa-check-square" aria-hidden="true"></i>
								 </a>';
					}else if($rows['status']=='PUBLISH'){
						$button='<a class="btn btn-sm btn-danger" href="'.site_url('info/unpublish/'.trim(base64_encode($rows['id']),'=').'').'">
									<i class="fa fa-ban" aria-hidden="true"></i>
								 </a>';
					}else{
						$button='<a class="btn btn-sm btn-success" href="'.site_url('info/publish/'.trim(base64_encode($rows['id']),'=').'').'">
									<i class="fa fa-check-square" aria-hidden="true"></i>
								 </a>';
					}
					echo '<tr>';
					...
					echo '<td>
							'.$button.'
							<a class="btn btn-sm btn-primary" href="'.site_url('info/edit/'.trim(base64_encode($rows['id']),'=').'').'">
								<i class="fa fa-pencil" aria-hidden="true"></i>
							</a>
							<a class="btn btn-sm btn-danger" href="#" data-href="'.site_url('info/delete/'.trim(base64_encode($rows['id']),'=').'').'"  data-toggle="modal" data-target="#confirm-delete" >
