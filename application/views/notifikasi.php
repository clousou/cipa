		  ...
		  <table id="user" class="display table-bordered table-striped table-hover" cellspacing="0" width="100%">
			...
			if($mdata->num_rows()>0){
				...
				foreach($mdata->result_array() as $rows){
					echo '<tr>';
					echo '<td>'.$no.'</td>';
					echo '<td>'.tgl_indo($rows['tgl']).' '.$rows['waktu'].'</td>';
					echo '<td>'.$rows['nama'].'</td>';
					echo '<td>'.$rows['subjek'].'</td>';					
					echo '<td>'.$rows['pesan'].'</td>';					
					echo '<td>'.$rows['status'].'</td>';
					echo '<td>
							<a class="btn btn-sm btn-danger" href="#" data-href="'.site_url('notifikasi/delete/'.trim(base64_encode($rows['id']),'=').'').'"  data-toggle="modal" data-target="#confirm-delete" >
